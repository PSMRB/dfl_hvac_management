...
class Problem(u.Canonical):
    ...
    def _solve(self,
        ...
        s.LOGGER.info(
            'CVXPY file modified by Pietro Favaro.')
        ...
        # BEGIN: My change to the original code
        n_variables = int(sum(np.prod(v.shape) for v in self.variables()))
        n_constraints = int(sum(np.prod(c.shape) for c in self.constraints))
        n_parameters = int(sum(np.prod(p.shape) for p in self.parameters()))
        self.n_variables, self.n_constraints, self.n_parameters = n_variables, n_constraints, n_parameters
        # END: My change to the original code
        if verbose:
            s.LOGGER.info(
        ...
        # Solution is the return form the solver (the solver and its output) is different for each solver
        solution = solving_chain.solve_via_data(
            self, data, warm_start, solver_verbose, kwargs)
        # Hereunder you extract all the data from solution that you wish. Once solution goes in self.unpack_results
        # it is in-place modified.
        # BEGIN change
        if solver == "MOSEK":
            import mosek
            extra_stats2 = {}
            solution["task"].analyzeproblem(mosek.streamtype.msg)
            mip_gap = solution["task"].getdouinf(mosek.dinfitem.mio_obj_rel_gap)
            nb_var = solution["task"].getintinf(mosek.iinfitem.ana_pro_num_var)
            nb_var_bin = solution["task"].getintinf(mosek.iinfitem.ana_pro_num_var_bin)
            nb_var_cont = solution["task"].getintinf(mosek.iinfitem.ana_pro_num_var_cont)
            nb_var_int = solution["task"].getintinf(mosek.iinfitem.ana_pro_num_var_cont)
            nb_const = solution["task"].getintinf(mosek.iinfitem.ana_pro_num_con)
            nb_param = n_parameters  # could not extract that from mosek directly
            extra_stats2 = {"mip_gap": mip_gap, "nb_var": nb_var, "nb_var_bin": nb_var_bin, "nb_var_cont": nb_var_cont,
                            "nb_var_int": nb_var_int, "nb_const": nb_const, "nb_param": nb_param}
        # END change

        end = time.time()
        self._solve_time = end - start
        # unpack results set self._solution and self._status and self._solver_stats
        ...
        # BEGIN change
        if solver == "MOSEK":
            self.solver_stats.extra_stats.update(extra_stats2)
        # END change
        return self.value
