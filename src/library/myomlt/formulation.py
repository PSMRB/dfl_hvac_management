...
def _setup_scaled_inputs_outputs(block, scaler=None, scaled_input_bounds=None):
    if scaled_input_bounds is not None:
        ...
        block.scaled_inputs = pyo.Var(
            block.inputs_set, bounds=bounds_rule
        )  # , initialize=[15.17, 466.6, 0, 383.17, 15.16]
    else:
        block.scaled_inputs = pyo.Var(block.inputs_set)  # , initialize=[15.17, 466.6, 0, 383.17, 15.16]
