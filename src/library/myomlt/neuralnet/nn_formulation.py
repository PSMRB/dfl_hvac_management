...
from ..formulation import _PyomoFormulation, _setup_scaled_inputs_outputs
from .activations import ACTIVATION_FUNCTION_MAP as _DEFAULT_ACTIVATION_FUNCTIONS
from .activations import (
    ...
    bigm_leakyrelu_activation_constraint,
...
from .layer import ConvLayer2D, DenseLayer, InputLayer, PoolingLayer2D
from .layers.full_space import (
    full_space_conv2d_layer,
    full_space_dense_layer,
    full_space_maxpool2d_layer,
)
from .layers.partition_based import (
    default_partition_split_func,
    partition_based_dense_relu_layer,
)
from .layers.reduced_space import reduced_space_dense_layer
...
_DEFAULT_ACTIVATION_CONSTRAINTS = {
    ...
    "leakyrelu": bigm_leakyrelu_activation_constraint,
...
def _build_neural_network_formulation(
    ...
    def layer(b, layer_id):
        ...
        if isinstance(net_layer, InputLayer):
            for index in net_layer.output_indexes:
                ...
                z_var = b.z[index]
                # TODO: Check that the bounds computation is valid for leaky relu
    ...
    for layer in layers:
        ...
        # layer_constraints is a dictionary with the functions for the constraints as values
        ...
        # activation_constraints is a dictionary with the functions for the constraints as values
...
class ReluIdealFormulation(FullSpaceNNFormulation):
    def __init__(self, network_structure):
        """
        This class is used for building "full-space" formulations of
        neural network models composed of relu activations using an
        Ideal formulation. See paper "Strong Mixed-Integer Programming formulations for trained neural networks" by
        Ross Anderson, Joey Huchette, and Juan Pablo Vielma
    ...
    def _supported_default_activation_constraints(self):
        return {
            "linear": linear_activation_constraint,
            "relu": ideal_relu_activation_constraint,
        }


class ReluExtendedFormulation1(FullSpaceNNFormulation):
    def __init__(self, network_structure):
        """
        This class is used for building "full-space" formulations of
        neural network models composed of relu activations using an
    ...
    def _supported_default_activation_constraints(self):
        return {
            "linear": linear_activation_constraint,
            "relu": extended_relu_activation_constraint1,
        }


class ReluExtendedFormulation2(FullSpaceNNFormulation):
    def __init__(self, network_structure):
        """
        This class is used for building "full-space" formulations of
        neural network models composed of relu activations using an
    ...
    def _supported_default_activation_constraints(self):
        return {
            "linear": linear_activation_constraint,
            "relu": extended_relu_activation_constraint2,
        }
...
class LeakyReluBigMFormulation(FullSpaceNNFormulation):
    ...
    def _supported_default_activation_constraints(self):
        return {
            "linear": linear_activation_constraint,
            "leakyrelu": partial(bigm_leakyrelu_activation_constraint, alpha=self.alpha)
        }
